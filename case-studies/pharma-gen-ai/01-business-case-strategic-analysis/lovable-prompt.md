...
New Analysis — target company (fictional OK), therapeutic area dropdown, Licensing/Acquisition/Partnership selector.
Research Findings — AI-generated table (Category | Finding | Source | Confidence) under an "AI-generated — review before proceeding" banner; each row needs a Reviewed check or explicit Dismiss before continuing (dismissed rows are excluded downstream).
Financial Inputs — five analyst-entered fields (peak sales, PTRS, discount rate, deal cost, timeline) with validation; deliberately never pre-filled from AI output.
Financial Model — rNPV computed deterministically so the displayed formula (rNPV = Σt [PeakSales(t) × Margin × PTRS ÷ (1+r)^t] − DealCost), input table, low/base/high range, and year-by-year cash-flow table always agree; Lovable AI writes the narrative, sensitivity commentary, and deal economics.
Risk Classification — AI Low/Medium/High verdict with written justification and key drivers, locked behind a "Confirmed by Compliance" checkbox (re-running classification resets the sign-off).
Draft Business Case — AI assembles all seven sections from only the reviewed findings and prior outputs, under a persistent DRAFT — Pending Human Review banner and striped watermark; "Mark as Final" stays disabled until a reviewer name is entered, then flips to a green FINAL banner.
